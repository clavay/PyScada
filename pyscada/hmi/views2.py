...
from pyscada.utils import gen_hiddenConfigHtml, get_group_display_permission_list, timestamp_to_datetime
...
async def aget_cache_data(
    ...
    timestamp_from = float(request.GET.get("timestamp_from", 0.0))
    timestamp_to = float(request.GET.get("timestamp_to", time.time() * 1000))

    response = StreamingHttpResponse(
        streaming_content=cache_data(
...
async def aget_last_data(
    ...
    response = StreamingHttpResponse(
        streaming_content=last_element(
            timestamp_from=timestamp_from,
            timestamp_to=timestamp_to,
            active_variables=active_variables,
            active_variable_properties=active_variable_properties,
            last_id=last_id,
        ),
        content_type="text/event-stream",
    )
...
async def aget_new_data(
    request: HttpRequest,
) -> StreamingHttpResponse:
    # get new data when it comes on redis channel layer
    last_id = request.headers.get("Last-Event-ID")
    response = StreamingHttpResponse(
        streaming_content=get_channel_message(last_id=last_id),
        content_type="text/event-stream",
    )
    response["Cache-Control"] = "no-cache"
    response["X-Accel-Buffering"] = "no"
...
async def cache_data(
    ...
    async for recordedElement in RecordedData.objects.filter(
        id__range=(
            (timestamp_from - 3660) * 2097152,
            timestamp_to * 2097152,
        ),
        date_saved__range=(
            timestamp_to_datetime(timestamp_from/1000),
            timestamp_to_datetime(timestamp_to/1000),
        ),
        variable_id__in=active_variables,
    ).aiterator():
        message = await recordedElement.get_variable_time_value_date_saved()
        logger.info(message)
        yield sse_message(
            event="cache_data",
            event_id=last_id + 1,
...
async def last_element(
    ...
    active_variables = [int(pk) for pk in active_variables]
    active_variable_properties = [int(pk) if pk != "" else None for pk in active_variable_properties]
    ...
    async for variable in variables:
        if await sync_to_async(variable.query_prev_value)(timestamp_from / 1000):
            # add 5 seconds to let the request from the server to come
            if (
                variable.timestamp_old is not None
                and variable.timestamp_old <= timestamp_to / 1000 + 5
    ...
    async for vp in VariableProperty.objects.filter(
        ...
        data = {"type": "variable_property", "id": vp.id, "timestamp": vp.last_modified.timestamp() * 1000, "value": vp.value(), "date_saved": vp.last_modified.timestamp()}
...
async def get_channel_message(last_id: int | None = None):
    last_id = 0 if last_id is None else int(last_id)
    logger.info(last_id)

    channel_layer = channels.layers.get_channel_layer()
    logger.info(channel_layer)
    try:
        async for message in get_channel_layer_data(channel_layer):
            logger.info(sse_message(
                event="new_data",
                event_id=last_id + 1,
                data=json.dumps(message),
            ))
...
class get_channel_layer_data:
    def __init__(self, channel_layer=channels.layers.get_channel_layer(), room="PyScadaFromDevice"):
    ...
    async def __anext__(self):
        # wait for message
        logger.info("waiting")
        message = await self.channel_layer.receive(self.room)
